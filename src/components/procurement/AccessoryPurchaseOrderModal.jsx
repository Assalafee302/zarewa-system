...
export default function AccessoryPurchaseOrderModal({
  ...
  const lineTotals = useMemo(
    () => lines.map((l) => (Number(l.qty) || 0) * (Number(l.unitPrice) || 0)),
    [lines]
  );
  ...
  return (
    <ModalFrame
      isOpen={isOpen}
      onClose={onClose}
      title={editPoId ? 'Edit accessory purchase order' : 'New accessory purchase order'}
      description="Accessory stock: ACC-* SKUs, quantity and unit price."
    >
      <div className="z-modal-panel max-w-[min(100%,min(96vw,42rem))] w-full max-h-[min(92vh,820px)] flex flex-col mx-auto">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-widest mt-0.5">
                Quantity × unit ₦ · ACC-* catalog SKUs
              </p>
          ...
          <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-white min-h-0 flex flex-col gap-6">
            ...
            >
              <p className="text-[9px] text-slate-500 mb-2 leading-snug">
                Pick an <strong>ACC-*</strong> product, quantity, and unit price. Line amount is qty × unit ₦.
              </p>
              <div className="space-y-2">
                {lines.map((row, idx) => (
                  <div
                    key={row.rowUid}
                    className="rounded-lg border border-slate-200/90 bg-white p-2 shadow-sm grid grid-cols-1 sm:grid-cols-12 gap-2 items-end sm:min-w-0"
                  >
                    <div className="sm:col-span-5 min-w-0">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className={`${lineInputClass} appearance-none pr-6 min-w-0 max-w-full`}
                        >
                    ...
                    <div className="sm:col-span-3 flex flex-col gap-0.5 min-w-0">
                      ...
                      <div className="flex flex-nowrap items-center justify-end gap-1.5 min-h-[1.625rem] min-w-0">
                        <p className="text-[10px] font-bold text-[#134e4a] tabular-nums flex-1 truncate text-right min-w-0">
                        ...
                        <button
                          ...
                          title="Add accessory line"
                          aria-label="Add accessory line"
                        >
