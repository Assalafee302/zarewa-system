 ...
 * - Cutting lists require ≥50% paid toward the quote: the gate uses `amountDueOnQuotationFromEntries` (row `paid_ngn`
 *   plus ledger receipts/advances without double-counting when the row already reflects posted ledger amounts).
 */
...
/** Quotation with ≥50% paid via ledger receipt (satisfies cutting-list gate without PATCH). */
async function freshPaidQuotationForCutting(agent, unitPriceNgn = 400_000) {
  ...
  const payNgn = Math.max(Math.ceil(total * 0.5), 1);
...
describe('Inventory scenarios (simulated flows)', () => {
  ...
  it('S1 — GRN: coil remaining and COIL-ALU stock increase together', async () => {
    ...
    await seedOneCoil(agent, 'CL-INV-S1', 2000);
